function R_tilde_ub = vertical_calculate_R_tilde_ub(M,N,T,S,A,Distance_xy,Height_l,Power,L_0,N_0)
   ...
   for t = 1:T
       for n = 1:N
           for m = 1:M
               ...
               for ss = 1:N-1
                   H_without_n(ss) = Height_l(m,t);
               end
               A_without_n = A(:,:,t);
               P_without_n =Power(t,:);
               Distance_wihout_n = Distance_xy(m,:,t);
               Distance_wihout_n(:,n) = [];
               P_without_n(:,n) =[];
               A_without_n(:,n) = [];
